...
ui <- fluidPage(
    ...
    visNetworkOutput("visNetworkRobustness"),
    # switch to highrisk
    ...
    # edge width selector
    selectInput("edgewidth", "Edge width by", choices=c("Robustness", "Robustness (inverse)", "Sample size", "Number of studies", "Equal size"), selected="Robustness"),
    # slider filter robustness
    sliderInput("filterrobustness", "Fliter contrasts by robustness: ",
                min=0,
                max=1,
                value=c(0,1)),
...
server <- function(input, output, session) {
            ...
            renderVisNetwork({
                plot %>%
                    ...
                    visEvents(click = "function(properties) {
                            ...
                            Shiny.onInputChange('undoedgeid', properties.edges);
                            ;}" 
                    )
            })
        )
    ...
    updatenodes <- function(size){
        ...
        visNetworkProxy("visNetworkRobustness") %>% visUpdateNodes(nodes)
    }
    ...
    updatanewargument <- function(highrisk=F, edgesizeby=0, sliderfilter=NULL, hover.edge=NULL, select.edge=NULL, tabledeselectededge=NULL, tableselectededge=NULL){
        ...
        visNetworkProxy("visNetworkRobustness") %>% visUpdateEdges(edges)
    }
