...
networkrobustbessUI <- function(id) {
  ...
  # invoke later.
    ...
    # reset button for network robustness plot
      ...
      # edge width selector
      selectInput(ns("edgewidth"), "Edge width by", choices=c("Robustness", "Robustness (inverse)", "Sample size", "Number of studies", "Equal size"), selected="Robustness"),
      # slider filter robustness
      sliderInput(ns("filterrobustness"), "Fliter contrasts by robustness: ",
                  min=0,
                  max=1,
                  value=c(0,1)),
...
networkrobustbessServer <- function(id, stringsAsFactors){
  moduleServer(
    id,
    ...
    function(input, output, session) {
      ...
      # get data
      plot <- get.interactive.contrast_robust_plot(indata, hatmatrix, highrisk=F, nodesizeby=1, edgesizeby = 0)
      data <- plot$x
      nodes <- data$nodes
      edges <- data$edges
          ...
          return(
              renderVisNetwork({
                  plot %>%
                      ...
                      # click an edge
                      visEvents(click = sprintf('function(properties) {
                              Shiny.onInputChange("%s", properties.edges);
                                ;}', ns("current_edges_selection")), hoverEdge = sprintf('function(edges) {
                              Shiny.onInputChange("%s", edges.edge);
                              ;}', ns("current_edge_hover")), blurEdge = sprintf('function(edges) { 
                              Shiny.onInputChange("%s", 0);
                              ;}', ns("current_edge_hover")), doubleClick = sprintf('function(properties) {
                              Shiny.onInputChange("%s", properties.edges);
                              ;}', ns("undoedgeid")) 
                      )
              })
      ...
      updatanewargument <- function(highrisk=F, edgesizeby=0, sliderfilter=NULL, hover.edge=NULL, select.edge=NULL, tabledeselectededge=NULL, tableselectededge=NULL){
          # highrisk switcher
          if(highrisk==T){
          ...
          if(length(hover.edge)>0){
              edges[ which( edges$id == hover.edge ), ]$width <- edges[ which( edges$id == hover.edge ), ]$width + 3
      ...
      observeEvent(input$filterrobustness, {
          ...
          filterrobust$value <- which((edges$contrast_robust > input$filterrobustness[2]) | (edges$contrast_robust < input$filterrobustness[1]))
      ...
      observeEvent(input$current_edge_hover, {
          ...
          if(input$current_edge_hover != 0){
              ...
              hover_edge <- edges[ which(edges$id == myEdgehover$hovered), ]
      ...
      dt_visselect <- function(select, hidden){
          temp <- edges[ which( ( edges$id %in% select == T) & (edges$id %in% hidden == F) ), ]
      ...
      dt_visunselect <- function(select, hidden){
          temp <- edges[ which( ( edges$id %in% select == F) & (edges$id %in% hidden == F) ), ]
          out <- data.frame(Contrast=temp$contrast, 
                            Robustness=round(temp$contrast_robust, 3),
                            Mean_stream_length=round(temp$meanStreamLen, 3),
                            Number_of_streams=temp$NumberStream,
